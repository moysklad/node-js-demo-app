...
import "../globals";
import { readLoyaltyConnectResponse } from "./response";
...
authDialog?.addEventListener("click", (event) => {
  ...
  const clickedOutside =
    (event as MouseEvent).clientX < bounds.left ||
    (event as MouseEvent).clientX > bounds.right ||
    (event as MouseEvent).clientY < bounds.top ||
    (event as MouseEvent).clientY > bounds.bottom;
  if (clickedOutside) closeAuthDialog();
});
...
manualDialog?.addEventListener("click", (event) => {
  ...
  const clickedOutside =
    ...
    (event as MouseEvent).clientY < bounds.top ||
    (event as MouseEvent).clientY > bounds.bottom;
  if (clickedOutside) closeManualDialog();
});
...
manualForm?.addEventListener("submit", (event) => {
  ...
  manualStatus.classList.remove("is-visible");
  manualStatus.textContent = "Сохраняем настройки...";
  manualRequest.hidden = true;
  manualFinishActions.hidden = true;
