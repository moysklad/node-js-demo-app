...
- Backend удаляет токен из разобранного тела запроса и вызывает только `POST {MOYSKLAD_VENDOR_API_ENDPOINT_URL}/context/user` под service JWT (`vendorJWT`).
- Zeus возвращает `{accountId,userId,userUid,role}`. Допустимые роли: `admin`, `cashier`, `worker`, `individual`.
...
`isAdmin` равен `true` только для роли `admin`. Роли `cashier` и `worker` не дают права менять настройки. Роль `individual` описывает тип пользовательского контекста, но контракт не обещает для нее административных полномочий, поэтому она также отображается без прав администратора.

Логирование входящих и исходящих запросов редактирует поля `token` и другие секреты во всех форматах логов. Ошибки Zeus передаются в UI безопасно: сохраняется HTTP-статус и, если он присутствует, код ошибки, но не тело ответа и не токен.
