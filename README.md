...
- Подключение минимального провайдера Loyalty API с настраиваемым поиском покупателей
...
- `LOYALTY_API_ENABLED` (`optional`, default: `false`) — при значении `true` включает Loyalty API и выбирает `/entry/loyalty` вместо обычного `/entry/iframe` в дескрипторе.
...
Решение подключает программу лояльности через `Vendor API`. На странице `/entry/loyalty` сначала показывается короткое объяснение и кнопка `Пример реализации`, а форма подключения открывается только по явному клику. В примере есть вкладки `Авторизация`, `Регистрация` и `Форма ввода`, а также переключатель `externalSearch`, чтобы выбирать, где искать покупателей.
...
В SQLite сохраняются только данные установки и зашифрованный токен Loyalty API. При удалении решения эти данные подключения удаляются. Бизнес-данные программы лояльности пример не хранит. Вкладки авторизации и регистрации на экране — демонстрация сценария; реальная авторизация во внешней программе лояльности не выполняется.

Администратор на странице настроек может передать через Vendor API URL, токен и выбранный `externalSearch`. URL по умолчанию равен `${APP_BASE_URL}/loyalty`, но его можно изменить, например для публичного reverse proxy. После успешной передачи настроек демо переводит решение в статус `Activated`. Контракт заглушек проверяется интеграционными HTTP-тестами.
...
- Таблица `loyalty_installation` содержит режим поиска и зашифрованный токен подключения Loyalty API.
