...
Решение подключает программу лояльности через `Vendor API`. Точка встраивания живет во вкладке `Программа лояльности` основного iframe: у решения один основной iframe, поэтому отдельные разделы вендор организует внутри своей страницы сам.

Сначала на вкладке показывается состояние подключения, короткое объяснение и переключатель с примером `curl`-запроса, а примеры подключения открываются только по явному клику: форма интересна не всем вендорам, а только вендорам программ лояльности. В примерах два способа получить настройки — через авторизацию или регистрацию пользователя в программе лояльности (рекомендованный) и прямым вводом URL, токена и режима `externalSearch`.

Элемент `<loyaltyApi/>` в дескрипторе лишь открывает решению право передавать настройки лояльности через Vendor API: без него запрос `PUT /apps/{appId}/{accountId}/loyalty` отклоняется. Сам по себе он программу лояльности в аккаунте не включает, поэтому отдельная переменная окружения для него не нужна.

В `src/loyalty/router.ts` объявлены документированные методы:

- `POST /loyalty/counterparty` — заглушка регистрации покупателя;
- `GET /loyalty/counterparty` — внешний поиск покупателей по демонстрационной базе; при `externalSearch: false` метод отвечает `404`, поскольку внешний поиск не используется;
- `POST /loyalty/counterparty/detail` — возвращает нулевой бонусный баланс;
- `POST /loyalty/retaildemand/recalc` — возвращает позиции без скидки и нулевые начисление и списание;
- `POST /loyalty/retaildemand` — заглушка фиксации продажи;
- `POST /loyalty/retailsalesreturn` — заглушка фиксации возврата.

Необязательные методы, которые в примере не реализованы:

- `POST /loyalty/counterparty/verify` — нужен, если регистрация покупателя требует подтверждения кодом;
- `POST /loyalty/retaildemand/verify` — нужен, если `recalc` возвращает `needVerification: true` для списания бонусов;
- `GET /loyalty/giftcard` — нужен при поддержке подарочных сертификатов.

Это контрактный каркас, а не готовая бонусная система. Для использования в рабочем решении необходимо реализовать хранение участников и балансов, правила скидок, начисление и списание бонусов, обработку продаж и возвратов, идемпотентность и необходимые проверки.

В SQLite сохраняются только данные подключения: зашифрованный токен, режим поиска и отметка о том, что настройки приняты Vendor API. Бизнес-данные программы лояльности пример не хранит. Экран авторизации и регистрации — демонстрация сценария; реальная авторизация во внешней программе лояльности не выполняется.

Токен переживает удаление решения, как и остальные настройки, но отметка о подключении при повторной установке сбрасывается: МойСклад удаляет настройки лояльности вместе с решением, поэтому их нужно передать заново. Приостановка решения настройки лояльности не удаляет — после `Resume` подключение продолжает работать.

Администратор на вкладке может передать через Vendor API URL, токен и выбранный `externalSearch`. URL по умолчанию равен `${APP_BASE_URL}/loyalty`, но его можно изменить, например для публичного reverse proxy. Токен сохраняется до обращения к Vendor API: иначе при сбое МойСклад знал бы токен, которого нет у решения, и все запросы Loyalty API получали бы `401`.

Если Vendor API отклоняет настройки, решение показывает его код и текст ошибки, а для частых причин добавляет подсказку — например, при ошибке `2006` сообщает, что в дескриптор нужно добавить `<loyaltyApi/>`.

Подключение программы лояльности необязательно и на статус решения не влияет: `SettingsRequired`/`Activated` определяется обязательными настройками (в примере — выбранным складом).

ВНИМАНИЕ! Первый успешный `PUT .../loyalty` может сделать вашу программу лояльности активной скидочной системой аккаунта, если других активных скидок на нем нет. На рабочем аккаунте это меняет поведение Точки продаж, поэтому подключайте демо на тестовом аккаунте. Контракт заглушек проверяется интеграционными HTTP-тестами.
