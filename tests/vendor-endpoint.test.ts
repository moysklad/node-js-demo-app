...
function configureRepositories(initialRow: AppInstanceData): MemoryAppInstanceRepository {
  const repository = new MemoryAppInstanceRepository([initialRow]);
...
test("Suspend сохраняет настройки, Resume возвращает решение в работу", async () => {
  const repository = configureRepositories(activatedRow());
...
test("установка без сохраненных настроек требует настройки решения", async () => {
  const repository = configureRepositories({ ...activatedRow(), store: "", status: AppStatus.UNKNOWN });
  const server = await startTestServer();
